class UserEventsController < ApplicationController
  ...
  def index
    @user_events = UserEvent.where(user_id: current_user.id).order(start_date: :desc).limit(25)
  end
  ...
  private
    ...
    def user_event_params
      modified_params = params.require(:user_event).permit(:name, :start_date)
      modified_params['user_id'] = current_user.id if !current_user.nil?
